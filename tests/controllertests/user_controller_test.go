...
func TestGetUsers(t *testing.T) {
	...
	if err != nil {
		log.Fatalf("Unable to convert response to JSON: %v\n", err)
	}
...
func TestGetUserByID(t *testing.T) {
	...
	for _, v := range userSample {
		...
		if err != nil {
			t.Errorf("unable to setup request\n %v", err)
		}
...
func TestDeleteUser(t *testing.T) {
	...
	for _, v := range userSample {
		...
		handler.ServeHTTP(rr, req)
		log.Printf("Token \nid:%s\nTokenGiven:%s\nStatusCode:%d\nerrorMessage:%s\n", v.id, v.tokenGiven, v.statusCode, v.errorMessage)
		assert.Equal(t, rr.Code, v.statusCode)
